...
#   Returns: ['eno', 'owt']
def reverse_each_element(words):
...
def sort_by_last_letter(words):
    return sorted(words, key=lambda word: word[-1].lower())
...
#   Returns: [9, 44, 7, 7]
def greater_than(nums, min_num):
    return [num for num in nums if num > min_num]
...
#   Returns: ['apple', 'nut']
def words_shorter_than(words, max_num):
    return [word for word in words if len(word) < max_num]
...
#   Returns: True
def all_above(nums, min_num):
    return all([num > min_num for num in nums])
...
#   Returns: True
def all_below(nums, max_num):
    return all([num < max_num for num in nums])
...
def multiply_each_by(nums, multiplier):
...
#   Returns: 106
def values_summed(animals):
    return sum(value for key, value in animals.items())
...
def add_key_value_pair(animals, animal_key, animal_value):
    animals.update({animal_key: animal_value})
    return animals


# Method name: remove_key_value_pair
# Purpose: returns a dictionary with the key and value removed
...
def remove_key_value_pair(animals, animal_key):
    animals.pop(animal_key)
    return animals
...
def where_value_below(animals, max_num):
    return {key: value for key, value in animals.items() if value < max_num}
...
def where_key_starts_with(animals, letter):
    return {key: value for key, value in animals.items() if key.startswith(letter)}
    # return dict(filter(lambda animal: animal[0][0] == letter, animals.items()))
