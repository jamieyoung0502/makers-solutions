...
class PasswordManager():
    ...
    def add(self, service: str, password: str) -> None:
        if len(password) > 7 and any(char in '!@$%&' for char in password):
            self.passwords[service] = password

    def get_for_service(self, service: str) -> Union[str, None]:
        return self.passwords.get(service)

    def list_services(self) -> list:
        return self.passwords.keys()

# def password_is_long_enough(password: str) -> bool:
#     return len(password) > 7

# def has_special_char(password: str) -> bool:
#     return any(char in '!@$%&' for char in password)

# self.passwords.update({service: password}) if password_is_long_enough(
#     password) and has_special_char(password) else None
